<template>
	<div class="main-content">
			...
			<el-form class="center-form-pv" :style='{"width":"100%","margin":"0px 0 0","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"minHeight":"100vh","padding":"34px 20px 140px","boxShadow":"0 2px 0px rgba(0,0,0,.1)","top":"94px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20221203/1f4690773fd84e218e0ef7d69885ee30.png) no-repeat left top,url(http://codegen.caihongy.cn/20221203/1057db36d2aa4bd4b375aa0ccb752a40.png) repeat-x left top,#7ca065","display":"block","width":"240px","position":"fixed","right":"0","order":"1"}' >
					<div :style='{"flexWrap":"wrap","margin":"0 0px 10px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#fff","textAlign":"left","display":"inline-block","width":"100%","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">出借账号</label>
						<el-input v-model="searchForm.chujiezhanghao" placeholder="出借账号" clearable></el-input>
					</div>
					<div :style='{"flexWrap":"wrap","margin":"0 0px 10px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#fff","textAlign":"left","display":"inline-block","width":"100%","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">出借姓名</label>
						<el-input v-model="searchForm.chujiexingming" placeholder="出借姓名" clearable></el-input>
					</div>
				...
				<el-row :style='{"width":"100%","padding":"0","margin":"20px 0 10px","background":"none","display":"flex"}'>
					<el-button :style='{"border":"0px dotted #a347ea","cursor":"pointer","padding":"0 8px","boxShadow":"4px 4px 0px rgba(0,0,0,.1)","margin":"0 10px 10px 0","color":"#7ca065","minWidth":"68px","outline":"none","borderRadius":"0px","background":"rgba(124,160,101,.2)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('chujiezhe','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0px dotted #df2543","cursor":"pointer","padding":"0 8px","boxShadow":"4px 4px 0px rgba(0,0,0,.1)","margin":"0 10px 10px 0","color":"#d67081","minWidth":"68px","outline":"none","borderRadius":"0px","background":"rgba(223,37,67,.2)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('chujiezhe','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #18cfa5","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#18cfa5","borderRadius":"4px","background":"rgba(24,207,165,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('chujiezhe','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"0px solid #18cfa5","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#18cfa5","borderRadius":"4px","background":"rgba(24,207,165,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('chujiezhe','出租收入')" type="success" size="mini" @click="chuzushouruCrossAddOrUpdateHandler(scope.row,'cross','','','')">出租收入</el-button>
							<el-button :style='{"border":"0px solid #1fbfae","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#4d9be3","borderRadius":"4px","background":"rgba(77,155,227,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('chujiezhe','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"0px solid #dc333b","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#dc333b","borderRadius":"4px","background":"rgba(220,51,59,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('chujiezhe','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<chuzushouru-cross-add-or-update v-if="chuzushouruCrossAddOrUpdateFlag" :parent="this" ref="chuzushouruCrossaddOrUpdate"></chuzushouru-cross-add-or-update>
...
import chuzushouruCrossAddOrUpdate from "../chuzushouru/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      chuzushouruCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    chuzushouruCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.chuzushouruCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','chujiezhe');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.chuzushouruCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.chuzushouruCrossaddOrUpdate.init(row.id,type);
      });
    },
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.chujiezhanghao!='' && this.searchForm.chujiezhanghao!=undefined){
            params['chujiezhanghao'] = '%' + this.searchForm.chujiezhanghao + '%'
          }
           if(this.searchForm.chujiexingming!='' && this.searchForm.chujiexingming!=undefined){
            params['chujiexingming'] = '%' + this.searchForm.chujiexingming + '%'
          }
      this.$http({
        url: "chujiezhe/page",
        method: "get",
